...
echoW2<-trim(Sar.tun[[1]],age = 0:4)
echoE2<-trim(Sar.tun[[2]],age = 0:2)
...
mat(Sar)[1,]  <-   1   # change the mat so that biom index is treated as TSB

Sar.ctrl                    <- FLSAM.control(Sar,Sar.tun2)

#- Set the number of fishing mortalities to estimate-All fishing mortality states are free except oldest ages to ensure stablity
Sar.ctrl@states["catch unique",]    <- c(0:3,3)

Sar.ctrl@cor.F  <-2

#- Set the number of fishing random walks to estimate- Separate variance for recruitment and plus group
#Sar.ctrl@logN.vars[]      <- c(0, rep(2,dims(Sar)$age-1))
#Sar.ctrl@f.vars["catch unique",]    <- c(0,1,1,2,2) 
#

#- Set the number of catchabilities to estimate by index and age
#Sar.ctrl@catchabilities["Echo West",ac(0:3)]      <- c(1,2,3,4)          + 101
#Sar.ctrl@catchabilities["Echo East", ac(0:1)]      <- c(1,2)    + 201


#- Set the number of observation variances to estimate by index and age
Sar.ctrl@obs.vars["catch unique",ac(0:4)]        <- c(0,1,1,1,3) 
#Sar.ctrl@obs.vars["Echo West",ac(0:4)]            <- c(0,0,1,1,2)          + 101 
#Sar.ctrl@obs.vars["Echo East",ac(0:1)]            <- c(1,2)          + 201
#Sar.ctrl@obs.vars["Echo East Biomass",ac(0)]          <- 301
#
...
save(Sar,Sar.tun,Sar.ctrl,Sar.sam,file=file.path(outPath,"Sarfinal.RData"))

Sar.sam            <- FLSAM(Sar,Sar.tun,Sar.ctrl)
