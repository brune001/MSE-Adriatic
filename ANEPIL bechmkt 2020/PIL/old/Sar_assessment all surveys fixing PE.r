...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### 
#### leave out the eastern survey
Sar.tun2     <- FLIndices("Echo West"=Sar.tun[[1]],
                          "Echo East Biomass"= EchoEastBiomass[[1]])
...
#### need to change the initial parameters configurations
    fit0.2noechoE             <- FLSAM(Sar,Sar.tun2,Sar.ctrl,
                              starting.values=init.sam,
                              map=list(logSdLogN=as.factor(c(-1.5,NA))),
                              return.fit=T)

   SAR.sam0.2noechoE          <- FLSAM(Sar,Sar.tun2,Sar.ctrl2,
                              starting.values=init.sam,
                              map=list(logSdLogN=as.factor(c(-1.5,NA))))
