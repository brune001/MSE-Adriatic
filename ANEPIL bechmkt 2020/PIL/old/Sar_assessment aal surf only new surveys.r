...
Sar.ctrl@cor.F <-2
Sar.ctrl@catchabilities["Echo W17",ac(0:4)]<- c(0,1,1,1,1)
Sar.ctrl@catchabilities["Echo W18",ac(0:3)]<- c(0,1,1,1) +101
Sar.ctrl@catchabilities["Echo L"  ,ac(0:3)]<- c(0,1,1,2) +201
Sar.ctrl@catchabilities["Echo East"  ,ac(0:4)]<- c(0,1,2,2,2) +301
...
#############3 this shows that catchabilit for echo L is wild.. this + the fact that 
# the data show bearly any decline in abundance from 3 to 4...
# let's remove 4
Sar.tun[[3]]  <- trim(Sar.tun[[3]],age=0:2) #age 4 has only 1 datapoint so logical to drop

# does not work.... let's try to remove the weird age 0 in Eeastern
Sar.tun[[4]]  <- trim(Sar.tun[[4]],age=1:4) #age 4 has only 1 datapoint so logical to drop


Sar.ctrl                    <- FLSAM.control(Sar,Sar.tun)
Sar.ctrl@states [1,]    <- c(0,1,2,3,3)
Sar.ctrl@f.vars[1,] <- c(0,2,2,2,2)
Sar.ctrl@cor.F <-2
Sar.ctrl@catchabilities["Echo W17",ac(0:4)]<- c(0,1,1,1,2)
Sar.ctrl@catchabilities["Echo W18",ac(0:3)]<- c(0,1,1,1) +101
Sar.ctrl@catchabilities["Echo L"  ,ac(0:2)]<- c(0,1,1) +201
Sar.ctrl@catchabilities["Echo East"  ,ac(0:4)]<- c(-,1,2,2,2) +301
Sar.ctrl@catchabilities["Echo East Biomass"  ,ac(0)] <- 501

Sar.ctrl@obs.vars["catch unique",ac(0:4)]<- c(0,1,1,1,1)
Sar.ctrl@obs.vars["Echo W17",ac(0:4)]<- c(1,1,1,2,2) + 101
Sar.ctrl@obs.vars["Echo W18",ac(0:3)]<- c(0,0,1,1) +201
Sar.ctrl@obs.vars["Echo L"  ,ac(0:3)]<- c(0,0,1,1) +301
