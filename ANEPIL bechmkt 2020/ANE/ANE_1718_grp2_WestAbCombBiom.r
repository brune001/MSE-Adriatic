...
ANE.tun   <- readFLIndices(file.path(data.source, "Ane17_18_flWestAbCombBiom.dat"))
...
biomSurf@index[]  <- ANE.tun[[2]]@index

range(biomSurf)["startf"] <- range(ANE.tun[[2]])["startf"]
range(biomSurf)["endf"]   <- range(ANE.tun[[2]])["endf"]
...
ANE.tun                         <- FLIndices("Echo West"=trim(ANE.tun[[1]],age=1:2),
...
# Catchabilities
ANE.ctrl@catchabilities["Echo West",2:3]        <- c(1,2)
...
ANE.ctrl@logN.vars[]                        <- c(0,1,1)

#Bind the observation variances
#ANE.ctrl@obs.vars["catch unique",]          <- c(1,2,3)
#ANE.ctrl@obs.vars["Echo West",2:3]          <- c( 101, 101) #rep(101,3) #c(101, rep(102,2)) #c(101,102,103)
#ANE.ctrl@obs.vars["Echo East",1:3]          <- c(201,202,202)
#ANE.ctrl@obs.vars["Echo East Biomass",1]    <- 301
##

ANE.ctrl@residuals <- F
...
### ======================================================================================================
ANE.sam             <- FLSAM(ANE,ANE.tun,ANE.ctrl,return.fit=T)
