...
sr.res    <- window(sr.res , start = 2016)
sr.res[,"2016"]  <- 0
...
#
  ...
  stkTmp <- fwd(stkTmp, ctrl=ctrl, sr=sr, sr.residuals = exp(sr.res[,ac(ay:(ay+1))]), sr.residuals.mult = TRUE  ,maxF = 10) 
...
save(restosave,file = paste0("./Results/",species,"/simres/",sc,".NO_SAM_trueR_",it,"its_",fy,".RData"))
